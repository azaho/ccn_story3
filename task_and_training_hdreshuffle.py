...
parser.add_argument('--shift_amount', type=float, help='how much to shift tunings?', default=120)
...
class Model(Model):
    def __init__(self):
        ...
        # 2: R1 bias
        self.top_parameters = nn.Parameter(torch.tensor([0.2, -0.1, 0.2]))
    ...
    def forward(self, input, noise):
        ...
        self.W_x_ah = legi((self.R1_pref+self.R1_pref_changes).repeat(task_parameters["input_direction_units"], 1).T, self.IN_pref.repeat(len(R1_pref), 1)) * self.top_parameters[2]
