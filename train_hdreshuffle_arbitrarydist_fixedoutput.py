...
model_parameters.update({
    "model_name": "hdreshuffle_ad_foCTRNN",
...
class Model(Model):
    def __init__(self):
        ...
        del self.fc_x2ah, self.fc_h2ah, self.fc_h2y
        ...
        # 2: R1 bias
        # 3: output sin and cos curve magnitudes
        self.top_parameters = nn.Parameter(torch.tensor([2, -1, 0.1])/args.net_size*10)
    ...
    def forward(self, input, noise):
        ...
        self.W_x_y = torch.cat((torch.sin(self.R1_pref).unsqueeze(0), torch.cos(self.R1_pref).unsqueeze(0))) * self.top_parameters[2]
        ...
        #print(hstore.shape)
        output = (self.W_x_y@hstore.reshape(-1, len(R1_pref)).T).T.reshape(batch_size, -1, 2) #self.fc_h2y(hstore)
